...
{
    public partial class ClearLogForm : Form
    {
        ...
        private void InitializeUI()
        {
            ...
            {
                Text = "警告：清除数据库日志是一个敏感操作。请确保已经备份重要数据，并在非业务高峰期执行。",
            ...
            // 监听窗体大小改变事件
            this.Resize += ClearLogForm_Resize;
        }
        ...
        private void ClearLogForm_Resize(object sender, EventArgs e)
        {
            // 调整按钮位置使其保持居中
        ...
        private void ExecuteButton_Click(object sender, EventArgs e)
        {
            ...
            //// 显示结果面板
            //ShowResult("✅ 正式清理完成！成功释放了数据库日志空间。", Color.FromArgb(82, 196, 26));
            ShowResult("暂时不开放", Color.FromArgb(250, 173, 20));
        }

        private void TestButton_Click(object sender, EventArgs e)
        {
            ClearSQLLog clearSQLlog = new(true);
            double remainSize = clearSQLlog.ShrinkDatabaseLog();
            // 显示结果面板
            ShowResult($"测试清理完成！操作成功。剩余大小{remainSize}MB", Color.FromArgb(250, 173, 20));
        }
